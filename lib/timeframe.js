...
console.log("categories", this.categories[0]);
	...
	Timeline.prototype.drawEvent = function (event) {
        ...
        var eventColor = (!event.color ? "black" : event.color);
		var group = $svg.group()
			.attr("class", "event")
			.appendTo(this.svg);
		...
		y =	this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 30;
		var label = $svg.text(
			x,
			y,
			event.name,
		...
		y =	this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 18;
		var label = $svg.text(
			x,
			y,
			event.reference,
        ...
        // Display the formatted year
		y =	this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 8;
		var year_text = Math.abs(eventYear) + (eventYear < 0 ? " BC" : " AD");	
		var year = $svg.text(
			x,
			y,
			year_text,
	...
	Timeline.prototype.drawSpan = function (span, color = "black", textColor = "black", spanOffset = 0) {
		...
		var spanOffsetHeight = -44 - this.options.textHeight;
		...
		startY = this.height - (this.options.textHeight * 2) - this.options.tickHeight + this.paddingY - 22 + (spanOffset * spanOffsetHeight);
		var label = $svg.text(
			(startX + endX) / 2,
			startY,
			span.name,
		...
		startY = this.height - (this.options.textHeight * 2) - this.options.tickHeight + this.paddingY - 10 + (spanOffset * spanOffsetHeight);
		var label = $svg.text(
			(startX + endX) / 2,
			startY,
			span.reference,
